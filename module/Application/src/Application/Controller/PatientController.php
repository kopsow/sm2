...
class PatientController extends AbstractActionController
{
    ...
    public function getRegistration($id)
    {
        ...
        $statement = $dbAdapter->query('SELECT registration.id,patient_id,users.name,users.surname,visit_date,registration_date,completed FROM registration LEFT JOIN users ON users.id = (SELECT user_id FROM physician WHERE id=registration.physician_id) WHERE patient_id=(SELECT id FROM patient where user_id='.$id.') AND DATE(visit_date)>="'.date('Y-m-d').'"');
